...
**Export for Klaviyo:** `outputs_finals/crm_treatment_tiers.csv` — **4,290 rows** with `crm_treatment_tier` (T1–T5), `retention_budget_5pct_sgd`, `retention_budget_10pct_sgd`, `max_sample_cost_sgd`. Generated by `build_crm_tiers_and_timing.py`.
...
| D5 | S$9 | S$0.44 | S$0.88 |

| Treatment tier | N | Avg 5% | Avg 10% | Incentive |
|----------------|---|--------|---------|-----------|
| **T1 VIP** | 532 | **S$12.91** | **S$25.82** | Partner rewards, HYROX/events, merch |
| T2 | 744 | S$5.85 | S$11.69 | Sub offer, shaker bundle |
| T4 | 2,120 | S$2.54 | S$5.08 | Sachet cap |
| T5 | 831 | S$0.44 | S$0.88 | Email only |
...
| Cold start | Weak for 67% one-and-done | **Rule-based** from 53–65% D1 co-purchase |
...
Shopify is fine for **Layer 2** (PDP widgets). Layers 1, 3, 4 and subscription timing **require Klaviyo + custom rules** from this analysis — they cannot be derived from Shopify alone.
...
#### Founder answer: sample at order vs before reorder?

| Moment | Use for | Example (Clear Protein buyer) |
|--------|---------|-------------------------------|
| **At first order** | Same-category bundle only (L2) | Peach + White Grape in cart — not Collagen |
| **Day 14 after delivery** | Email: recommend Lean for order 2 | CS-01 Klaviyo flow |
...
Physical cross-category samples ship **before the reorder window**, not in the first order box — so the customer experiences a new category before deciding to repurchase.

**Full matrix:** `outputs/cross_sell_timing_and_samples.csv` · `CRM_AND_INCENTIVES.md` §5
...
| Layer / flow | Subscriber mechanism |
|--------------|---------------------|
| L3 day 14 | Category cross-sell → 2nd order → qualifies for SUB-01 |
| L3 day 7 (order 2) | 3rd category → higher repeat → sub attach |
| **SUB-01** | Order 2 + **48 days** → S&S on hero SKU (Peach/TMT) |
| **SUB-02** | Freq D1 non-subs (228) → order 3 + 35 days |
| T1 VIP | 54% already sub — grow remainder with VIP S&S (no deep discount) |
| T2 | Primary sub conversion pool (744) — bundle + replenishment offer |
